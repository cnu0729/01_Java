...
/*
 메서드(method)
 	...
 	Getter: Setter에 저장된 변수값을 다른 곳에서 사용할 수 있도록 전닿ㄹ해주는 메서드
 		public 자료형 get 변수명(){
 			return 변수명;
 		}
 	...
 	생성자 : 파일명과 클래스 이름과 똑같이 작성을 해줘야하고
 		...
 		public 클래스이름(자료형 매개변수, 자료형 매개변수2){
 			...
 			결혼기념일 광고수신동의는 선택으로 하고싶다면 필수로 이름 나이를 (   ) 넣어줌
 			public 클래스이름 (String이름, String나이){
 				this.이름 = 이름;
 				this.나이 = 나이;
 			}
 		...
 		void : 어떤 행동을 수행하기 위해 작성하고, 수행한다음 특별히 전달할 내용이 없을 때 사용
 			   ...
 			   특정 행동을 하고 난 다음에 행동을 끝내는 것이지 값을 전달하거나 추가로 어떤 행동을
 		...
 		//문자열을 전달 = String
 		public String 특정행동	1(){
 			return "안녕하세요"; //위에 String을 작성했으니 return 다음에 String을 작성
 		}
 		
 		public int 특정행동2(){
 			return1; //public 옆에 int 를 작성했으니 return 다음에 숫자값을
 		}
